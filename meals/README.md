...
backed by its own Firebase project, kept deliberately separate from the
hospital rota app that lives in the rest of this repo — different
project, different billing, none of your personal data anywhere near
work infrastructure.
...
2. Pick any region close to you, and start in **production mode** (the
   rules in `meals/firestore.rules` handle access control — you don't
   need "test mode").

## 4. Add your config to the app

Open `meals/js/firebase-config.js` and replace the placeholder object
...
login`). Then, from inside the `meals/` folder:
...
Pages, Netlify, Vercel, …) — the `meals/` folder is entirely self-
contained static HTML/CSS/JS, nothing Firebase-specific about the
hosting beyond `firebase.json`.
